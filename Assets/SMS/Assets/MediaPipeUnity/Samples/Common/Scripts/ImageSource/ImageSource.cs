    ...
    public abstract class ImageSource
    {
        ...
        public struct ResolutionStruct
        {
            ...
            public ResolutionStruct(Resolution resolution)
            {
                ...
                // 🛠️ [수정] 구버전 resolution.refreshRate 대신 신버전 refreshRateRatio.value 적용
                frameRate = resolution.refreshRateRatio.value;
            }

            public Resolution ToResolution()
            {
                // 🛠️ [수정] 신버전 Resolution 생성 방식 대응
                return new Resolution()
                {
                    width = width,
                    height = height,
                    refreshRateRatio = new RefreshRate() { numerator = (uint)Math.Round(frameRate * 1000), denominator = 1000 }
                };
            }
