    ...
    public class UnityAndGeminiV3 : SingletonBase<UnityAndGeminiV3>
    {
        ...
        private void ParseEvaluationResult(string evaluationResult, InterviewResultData resultData)
        {
            try
            {
                ...
                Debug.Log($"[GeminiManager] 음성 점수 추출값: '{voiceScoreStr}'");
                ...
                Debug.Log($"[GeminiManager] 내용 점수 추출값: '{contentScoreStr}'");
        ...
        private int ParseScore(string scoreStr)
        {
            ...
            // AI가 소수점으로 반환했을 경우 안전장치
            if (float.TryParse(scoreStr,
                System.Globalization.NumberStyles.Float,
                System.Globalization.CultureInfo.InvariantCulture,
                out float floatScore))
                return Mathf.Clamp(Mathf.RoundToInt(floatScore), 0, 5);

            // 파싱 완전 실패 시
            Debug.LogWarning($"[GeminiManager] 점수 파싱 실패: '{scoreStr}'");
            return 0;
        }
