...
//   - start_time 제거
//   - end_time 유지 및 duration_seconds 물리 컬럼 추가
//
...
namespace InterviewDb.Core
{
    public static class SchemaBootstrapHardened
    {
        private static readonly string[] SchemaStatements = new string[]
        {
            ...
            // ── Interview_Session: interview_lang 제거, end_time 유지, duration_seconds 컬럼 신설 ──
            ...
            ON Interview_Session(end_time DESC);",
            ...
            // ── 4. View_Session_Report: end_time, duration_seconds 매핑 ──
            ...
            BEGIN
                ...
                -- Gemini의 'model', 'user' 소문자도 안전하게 통과되도록 조건 추가
            ...
            WHERE json_valid(NEW.conversation_log)
              AND EXISTS (
                  ...
                  WHERE LOWER(json_extract(value, '$.speaker')) NOT IN ('ai', 'user', 'model')
              );
            END;",
