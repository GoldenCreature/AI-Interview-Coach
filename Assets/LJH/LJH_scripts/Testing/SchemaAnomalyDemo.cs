...
//       있는 DB 이상현상(anomaly)들을 gilzoide(unity-sqlite-net) 위에서 재현.
//       Console에 [이상현상 발생]이 뜨면 실제로 재현된 것이고,
//       [정상 차단]이 뜨면 스키마 제약이 정상적으로 막아준 것.
//
// 재현하는 이상현상 목록:
...
//
// ⚠ 주의: 이 스크립트는 "의도적으로" 데이터 무결성을 깨뜨리므로,
//         반드시 실제 서비스 DB가 아닌 별도의 데모 전용 DB 파일에서만 실행해야 함.
// ============================================================
using System;
using System.Linq;
using System.IO;
