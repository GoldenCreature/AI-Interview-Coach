...
// ------------------------------------------------------------
// FaceEvaluationWriter가 실제로 기존 Session_Result/트리거와 잘 맞물리는지
// 확인하는 통합 테스트. 세 가지 상황을 모두 확인.
//
...
//    로그가 (a)/(b)보다 한 프레임 정도 늦게 찍힐 수 있는데, 이 경우도 정상 동작임.
// ============================================================
using System;
using System.IO;
...
namespace InterviewDb.Testing
{
    public class FaceEvaluationIntegrationTest : MonoBehaviour
    {
        ...
        private void Case_A_ExistingResult_ThenFaceUpdate(SQLiteConnection conn)
        {
            ...
            FaceEvaluationWriter.Save(conn, session.SessionId, 4.5, "미소가 아주 자연스럽습니다.");
            ...
            bool totalOk = result.TotalScore.HasValue &&
                Math.Abs(result.TotalScore.Value - Math.Round((4.0 + 4.0 + 4.5) / 3.0, 2)) < 0.0001;

            Debug.Log(attitudeOk && adviceOk && totalOk
                ? $"[PASS] (a) 기존 결과에 표정 점수 UPDATE 성공 → total_score={result.TotalScore} 자동 재계산됨, advice_text=\"{result.AdviceText}\""
                : $"[FAIL] (a) score_attitude={result.ScoreAttitude}, advice_text={result.AdviceText}, total_score={result.TotalScore}");
        }
        ...
        private void Case_B_FaceFirst_ThenInsert(SQLiteConnection conn)
        {
            ...
            FaceEvaluationWriter.Save(conn, session.SessionId, 3.5, "긴장한 표정이 자주 보였습니다.");
        ...
        private void Case_C_ViaBackgroundThreadAndDispatcher()
        {
            ...
            Task.Run(() =>
            {
                ...
                {
                    conn.Insert(session);
                    FaceEvaluationWriter.Save(conn, session.SessionId, 4.0, "표정 변화가 자연스럽습니다.");
