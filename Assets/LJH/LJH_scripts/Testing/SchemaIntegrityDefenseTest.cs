...
// ------------------------------------------------------------
// 현재(8차) 스키마(SchemaBootstrapHardened)가 실제로 방어하는지 검증.
//
//   [방어 성공] = 공격이 차단됨 / 데이터가 안전하게 유지됨 (원하는 결과)
//   [방어 실패] = 공격이 그대로 먹힘 (스키마를 더 보강해야 함)
//
// ⚠ App_Setting 테이블 제거, interview_lang 컬럼 제거, total_score 자동
//   계산 트리거 제거(8차 수정)에 맞춰 그 3개는 이 파일에서 뺐음.
//   total_score는 이제 "자동 계산되지 않는 것"이 올바른 동작이라, 그걸
//   확인하는 테스트로 바꿨음.
//
    ...
    public class SchemaIntegrityDefenseTest : MonoBehaviour
    {
        ...
        public void RunDefenseTest()
        {
            ...
            using (var conn = new SQLiteConnection(path))
            {
                ...
                Demo1_TotalScoreNotAutoCalculated(conn);
                Demo2_DomainAnomaly_InvalidStatus(conn);
                Demo3_TemporalAnomaly_EndBeforeStart(conn);
                Demo4_1NFAnomaly_InvalidLog(conn);
                Demo5_ReferentialAnomaly_TriggerGuard(path);
                Demo6_LostUpdate_NaiveVsOptimistic(path);
            }
        ...
        // ① total_score가 더 이상 자동 계산되지 않는지 확인 (8차 수정 반영 — 트리거 제거됨)
        private void Demo1_TotalScoreNotAutoCalculated(SQLiteConnection conn)
        {
            ...
            conn.Insert(new SessionResultHardened
            {
                ...
                // total_score는 일부러 채우지 않음
            });

            var afterInsert = conn.Get<SessionResultHardened>(session.SessionId);
            bool staysNullAfterInsert = !afterInsert.TotalScore.HasValue;
            ...
            bool staysNullAfterUpdate = !afterUpdate.TotalScore.HasValue;

            if (staysNullAfterInsert && staysNullAfterUpdate)
            {
                // 이제 이 시점에 SetTotalScore로 명시적으로 채워야 정상 흐름
                InterviewResultRepository.SetTotalScore(conn, session.SessionId, 4.33);
                var afterSet = conn.Get<SessionResultHardened>(session.SessionId);
                bool setWorks = afterSet.TotalScore.HasValue && Math.Abs(afterSet.TotalScore.Value - 4.33) < 0.0001;

                _log.Add(setWorks
                    ? "[방어 성공] total_score는 점수가 다 채워져도 자동 계산되지 않고, SetTotalScore로 직접 넣어야만 반영됨 (의도된 동작)"
                    : "[방어 실패] SetTotalScore로도 total_score가 반영되지 않음");
            }
            else
            {
                _log.Add("[방어 실패] total_score 자동 계산 트리거가 아직 남아있는 것으로 보임 (제거되었어야 함)");
            }
        }

        // ② 도메인 이상 — 정의되지 않은 session_status는 CHECK로 거부되어야 함
        private void Demo2_DomainAnomaly_InvalidStatus(SQLiteConnection conn)
        {
            try
            {
                conn.Insert(NewSession("금융", "완전히_잘못된_상태값", withEndTime: false));
        ...
        private void Demo3_TemporalAnomaly_EndBeforeStart(SQLiteConnection conn)
        {
            try
            {
        ...
        private void Demo4_1NFAnomaly_InvalidLog(SQLiteConnection conn)
        {
            // 4-1) 문법이 깨진 JSON
            try
            {
                var s1 = NewSession("디자인", "Completed", withEndTime: true);
            ...
            // 4-2) 문법은 맞지만 정의되지 않은 speaker 값("Robot")
            try
            {
                var s2 = NewSession("디자인", "Completed", withEndTime: true);
        ...
        private void Demo5_ReferentialAnomaly_TriggerGuard(string path)
        {
        ...
        private void Demo6_LostUpdate_NaiveVsOptimistic(string path)
        {
